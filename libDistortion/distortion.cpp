...
template <typename T>
void DistortedLines<T>::write2file(char *fname)
{
	FILE *pfile;
	...
	for (int k = 0; k < nImages; k++) {
		fprintf(pfile, "%i\n", nlines4Image[k]);
		for (int i = 0; i < nlines4Image[k]; i++)
		{
			fprintf(pfile, "%i\n", _line[idx].sizeLine());
			for (int j = 0; j < _line[idx].sizeLine(); j++)
				fprintf(pfile, "%f %f\n",_line[idx].x(j), _line[idx].y(j));
			idx++;
		}
	}
	fclose(pfile);
